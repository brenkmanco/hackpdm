...
directory(){
    ...
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> directory.sql << EOL
select
      '(' || dir_id  || ', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f directory.sql
}

entry(){
    echo $IS_HpEntry > entry.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> entry.sql << EOL
select
      '(' || entry_id  || ', '
      || dir_id || ', '
      || '''' || replace(entry_name, '''', '''''') || ''', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f entry.sql
}

release(){
    echo $IS_HpRelease > release.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> release.sql << EOL
select
      '(' || release_id  || ', '
      || umb.odoo_user_id || ', '
      || '''' || coalesce(
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f release.sql
}

version(){
    echo $IS_HpVersion > version.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> version.sql << EOL
select
      '(' || version_id  || ', '
      || entry_id || ', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f version.sql
}

vers_prop(){
    echo $IS_HpVersionProperty > vers_prop.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> vers_prop.sql << EOL
select
      '(' || version_id  || ', '
      || '''' || replace(config_name, '''', '''''') || ''', '
      || prop_id || ', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f vers_prop.sql
}

vers_relationship(){
    echo $IS_HpVersionRelationship > vers_relationship.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> vers_relationship.sql << EOL
select
    '(' || rel_parent_id  || ', '
    || rel_child_id || '),'
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f vers_relationship.sql
}

vers_rel(){
    echo $IS_HpReleaseVersionRel > vers_rel.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> vers_rel.sql << EOL
select
    '(' || rel_release_id  || ', '
    || rel_version_id || ', '
    || umb.odoo_user_id || ', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f vers_rel.sql
}

node(){
    echo $IS_HpNode > node.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> node.sql << EOL
select
    '(' || node_id  || ', '
    || '''' || replace(node_name, '''', '''''') || ''', '
    || umb.odoo_user_id || ', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f node.sql
}

entry_name_filter(){
    echo $IS_HpEntryNameFilter > entry_name_filter.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> entry_name_filter.sql << EOL
select
    '(' || filter_id  || ', '
    || '''' || replace(name_proto, '''', '''''') || ''', '
    || '''' || replace(name_regex, '''', '''''') || ''', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f entry_name_filter.sql
}
...
property(){
    ...
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> property.sql << EOL
select
    '(' || prop_id  || ', '
    || '''' || replace(prop_name, '''', '''''') || ''', '
    || '''' || replace(prop_type, '''', '''''') || ''', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f property.sql
}

#id, cat_id, file_ext, type_regex, description
type(){
    echo $IS_HpType > type.sql
    cat | psql -h $Target_Host -U $Target_DB -W -t $Target_DB >> type.sql << EOL
select
    '(' || type_id  || ', '
    || default_cat || ', '
    || '''' || replace(file_ext, '''', '''''') || ''', '
...
EOL
    ...
    psql -U $To_Host -d $To_DB -a -f type.sql
}
