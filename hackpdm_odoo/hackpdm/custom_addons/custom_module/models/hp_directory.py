...
class hp_directory(models.Model):
    ...
    #     store=True,
    # )
    sandboxed = fields.Boolean(string='sandboxed')
    ...
    #     # change complete name to a windows pathway and root to pwa
    #     # example:
    #     # from: 'root / Designed', and ' / ' + entry.name
    #     # to:   'pwa\Designed', and '\' + entry.name
    #     newpath = ""
    #     if path is str:
    #         split_directories = path.split(' / ')
    #         if split_directories[0] == "root" or split_directories[0] == "":
    #             split_directories[0] = "pwa"
    #         # should look like 'root / Categories'
    #         newpath = "\\".join(split_directories)
    #     return newpath
    ...
    def _get_all_entry_ids(self, directory, root_entry_list, withDeleted=True):
        ...
        for children_directory in directory.child_ids:
            self._get_all_entry_ids(children_directory, root_entry_list)

    @api.model
    def get_all_entry_ids(self, value, withDeleted=True, withSubEntries=True):
        entry_list = []
        try:
            record = self.env["hp.directory"].search([('id', '=', value)])[0]
            if record:
