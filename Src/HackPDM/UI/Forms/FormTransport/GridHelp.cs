	...
	internal class GridHelp
	{
				...
				SafeHelper.SafeInvokeGen(allProperties, (allp) =>
				{
					...
					foreach (HpVersionProperty[] versionProperties in allp)
					{
						...
						foreach (HpVersionProperty versionProp in versionProperties)
						{
							...
							{
								PropertyType.Yesno => versionProp.yesno_value,
								PropertyType.Number => versionProp.number_value,
								PropertyType.Text => versionProp.text_value,
								PropertyType.Date => versionProp.date_value,
								_ => null,
							};
			...
			lock (lockObject)
			{
				...
				SafeHelper.SafeInvokeGen(versions, (v) =>
				{
					...
					foreach (HpVersion version in v)
					{
						...
						if (moduser is not null) item.ModUser = OdooDefaults.Instance.IdToUser.TryGetValue(moduser ?? 0, out var user) ? user : null;
		...
		private void PopulateVersionInfo(DataGrid grid, HpVersion versionModel)
		{
			...
			lock (lockObject)
			{
				...
				SafeHelper.SafeInvokeGen(versionModel, (v) =>
				{
					InitListViewInternal(grid);
					...
					item.DirectoryId = versionModel.dir_id;
					item.NodeId = versionModel.node_id;
					item.EntryId = versionModel.entry_id;
					item.AttachmentId = versionModel.attachment_id;
					item.ModifyDate = versionModel.file_modify_stamp;
					string? path = versionModel.HashedValues != null && versionModel.HashedValues.TryGetValue("dir_id", out ArrayList? list)
						? (list?[1]?.ToString())
						: null;
		...
		internal static async Task<Dictionary<string, Task<HackFile>>> GetFileMap(Hashtable entries)
		{
			...
			foreach (DictionaryEntry pair in entries)
			{
				...
				Task<HackFile> hackTask = HackFile
					.GetHackFileAsync(FileOperations
