...
public sealed partial class MessageBox : Page
{
	...
	private MessageBoxRepresentation focusButton;
	...
	private void DefaultImpl()
	{
		...
		switch( _windowWidthWrap )
		{
			case MessageBoxWidth.DynamicWidthText:
			{
				...
				TextBlockImpl.TextWrapping = TextWrapping.Wrap;
				break;
			}
	...
	private (Button? primary, Button? secondary, Button? close) DefaultButtons()
	{
		...
		switch( _button )
		{
			case MessageBoxButtons.OK:
			{
				...
				focusButton = MessageBoxRepresentation.Primary;
				break;
			}
			...
			case MessageBoxButtons.AbortRetryIgnore:
			{
				...
				focusButton = MessageBoxRepresentation.Secondary;
				break;
			}
			...
			case MessageBoxButtons.CancelTryContinue:
			{
				...
				secondaryGrid = new( 3, 2, 1, 1 );
				closeGrid = new( 3, 3, 1, 1 );
				focusButton = MessageBoxRepresentation.Tertiary;
				break;
			}
		}
		...
		if( primary != null )
		{
			...
			primary.Style = ( Style )this.Resources[ focusButton is MessageBoxRepresentation.Primary
		...
		if( secondary != null )
		{
			...
			secondary.Style = ( Style )this.Resources[ focusButton is MessageBoxRepresentation.Secondary
				? "DarkButtonActiveStyle1"
				: "DarkButtonStyle1" ];
		...
		if( close != null )
		{
			...
			close.Style = ( Style )this.Resources[ focusButton is MessageBoxRepresentation.Tertiary
				? "DarkButtonActiveStyle1"
				: "DarkButtonStyle1" ];
	...
	private void PrimaryPress( object sender, RoutedEventArgs e ) => SetToClose( MessageBoxRepresentation.Primary );
	private void SecondaryPress( object sender, RoutedEventArgs e ) => SetToClose( MessageBoxRepresentation.Secondary );
	private void ClosePress( object sender, RoutedEventArgs e ) => SetToClose( MessageBoxRepresentation.Tertiary );
	private void SetToClose( MessageBoxRepresentation buttonPress )
	{
		Result = MapContentToDialogResult( buttonPress, _button );
		TCS.TrySetResult( Result ?? DialogResult.None );
		_messageWindow?.Close();
	}
	private static MessageBoxRepresentation GetDefaultButton( MessageBoxButtons buttons )
		=> buttons switch
		{
			MessageBoxButtons.OK				=> MessageBoxRepresentation.Primary,
			MessageBoxButtons.OKCancel			=> MessageBoxRepresentation.Secondary,
			MessageBoxButtons.AbortRetryIgnore	=> MessageBoxRepresentation.Primary,
			MessageBoxButtons.YesNoCancel		=> MessageBoxRepresentation.Tertiary,
			MessageBoxButtons.YesNo				=> MessageBoxRepresentation.Secondary,
			MessageBoxButtons.RetryCancel		=> MessageBoxRepresentation.Secondary,
			MessageBoxButtons.CancelTryContinue => MessageBoxRepresentation.Primary,
			_									=> MessageBoxRepresentation.Primary,
		};
	private static DialogResult MapContentToDialogResult( MessageBoxRepresentation result, MessageBoxButtons buttons ) =>
	(buttons: buttons, result) switch
	{
		(MessageBoxButtons.OK, MessageBoxRepresentation.Primary )					=> DialogResult.OK,
		(MessageBoxButtons.OK, MessageBoxRepresentation.Secondary )					=> DialogResult.OK,
		(MessageBoxButtons.OK, MessageBoxRepresentation.Tertiary )						=> DialogResult.OK,

		(MessageBoxButtons.OKCancel, MessageBoxRepresentation.Primary )				=> DialogResult.OK,
		(MessageBoxButtons.OKCancel, MessageBoxRepresentation.Secondary )			=> DialogResult.Cancel,
		(MessageBoxButtons.OKCancel, MessageBoxRepresentation.Tertiary )				=> DialogResult.Cancel,

		(MessageBoxButtons.AbortRetryIgnore, MessageBoxRepresentation.Primary )		=> DialogResult.Abort,
		(MessageBoxButtons.AbortRetryIgnore, MessageBoxRepresentation.Secondary )	=> DialogResult.Retry,
		(MessageBoxButtons.AbortRetryIgnore, MessageBoxRepresentation.Tertiary )	=> DialogResult.Ignore,

		(MessageBoxButtons.YesNoCancel, MessageBoxRepresentation.Primary )			=> DialogResult.Yes,
		(MessageBoxButtons.YesNoCancel, MessageBoxRepresentation.Secondary )		=> DialogResult.No,
		(MessageBoxButtons.YesNoCancel, MessageBoxRepresentation.Tertiary )			=> DialogResult.Cancel,

		(MessageBoxButtons.YesNo, MessageBoxRepresentation.Primary )				=> DialogResult.Yes,
		(MessageBoxButtons.YesNo, MessageBoxRepresentation.Secondary )				=> DialogResult.Cancel,
		(MessageBoxButtons.YesNo, MessageBoxRepresentation.Tertiary )				=> DialogResult.No,

		(MessageBoxButtons.RetryCancel, MessageBoxRepresentation.Primary )			=> DialogResult.Retry,
		(MessageBoxButtons.RetryCancel, MessageBoxRepresentation.Secondary )		=> DialogResult.Cancel,
		(MessageBoxButtons.RetryCancel, MessageBoxRepresentation.Tertiary )			=> DialogResult.Cancel,

		(MessageBoxButtons.CancelTryContinue, MessageBoxRepresentation.Primary )	=> DialogResult.Cancel,
		(MessageBoxButtons.CancelTryContinue, MessageBoxRepresentation.Secondary )	=> DialogResult.TryAgain,
		(MessageBoxButtons.CancelTryContinue, MessageBoxRepresentation.Tertiary )	=> DialogResult.Continue,
	...
	private static Task<DialogResult> ShowInternalAsync( string message, string? caption, MessageBoxButtons buttons, MessageBoxIcon icon, MessageBoxType type = MessageBoxType.Default, params object[]? parameters )
	{
		...
		catch( Exception ex )
		{
			tcs.SetResult(TryShowNative(
				...
				message,
				caption ?? "Information",
				// bitmask for native window argument: (uint) type
				(uint)buttons 
				| (uint)icon 
				| (uint)GetDefaultButton(buttons) 
				| (uint)MessageBoxModal.AppModal 
				| (uint)MessageBoxTypeFallback.TopMost,
				MsgBox));
		}	
		return tcs.Task;
	}
	private static DialogResult TryShowNative(
		IntPtr hwnd, 
		string message, 
		string title, 
		uint type,
		Func<nint, string, string, uint, int> mboxNativeMethod)
	{
		try
		{
			return ( DialogResult )mboxNativeMethod( hwnd, message, title, type);
		}
		catch 
		{
			Debug.WriteLine( $"{mboxNativeMethod.Method.Name} failed." );
		}
		
		throw new Exception( "Invalid data or functions" );
	}

	[LibraryImport( "User32.dll", EntryPoint = "MessageBoxW", StringMarshalling = StringMarshalling.Utf16 )]
	private static partial int MsgBox(
	IntPtr hWnd,
	string lpText,
	string lpCaption,
	uint uType );
