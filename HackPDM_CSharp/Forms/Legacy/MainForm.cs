...
namespace HackPDM
{
    ...
    public partial class MainForm : Form
    {
        ...
        public static ImgConvert imgConv = new();
        public OdooViewer OdooView;

        private string strDbConn;
        private NpgsqlConnection connDb = new();
        private NpgsqlTransaction t;

        private WebDAVClient connDav = new();

        private SWHelper connSwApi = new();
        ...
        private DataSet dsTree = new();
        private DataSet dsList = new();
        private List<string> lstSandboxed = [];

        private DataTable dtServSettings = new("settings");
        ...
        public MainForm(MainFormDirective directive)
        {
            switch (directive)
            {
                case MainFormDirective.LegacyMainForm:
                {
                    InitializeBase();
                    LoadProfile(false);
                    MainFormInitializer();
                ...
                case MainFormDirective.MainForm:
                {
                    InitializeBase();
                    LoadProfile(false);
                    MainFormInitializer();
                ...
                case MainFormDirective.ProfileManager:
                {
                    LoadProfile(true);
                    break;
                }
                case MainFormDirective.FileTypeManager:
                {
                    LoadProfile(false);
                    DbConnect();
                    new FileTypeManager( connDb, strLocalFileRoot, Utils.GetRelativePath( strLocalFileRoot, strLocalFileRoot ) ).Show();
                    break;
                }
                default: break;
            }
        }
        public MainForm() : this(MainFormDirective.LegacyMainForm) {}
        ...
        private void DbConnect()
        {
            ...
            NpgsqlCommand cmdGetId = new(strSql, connDb);
            try
            {
        ...
        private bool LoadProfileInternal(bool blnForceDlg, ref List<string> errors)
        {
            // load profile info
            ...
            try
            {
                ...
                var fileMap = new System.Configuration.ConfigurationFileMap("");
			...
			if ( blnForceDlg || strXmlProfiles == "" || strCurrProfileId == "" )
			{
				...
				ProfileManager dlgPM = new(errors);
				...
				{
                    errors.Add("\"Still can't get a profile.  Can't connect to the server.");
                    return false;
				}
			}
			...
			DataTable dtProfiles = new("profiles");
			...
			{
                errors.Add("Still can't get a profile.  Can't find the local pwa directory.");
                return false;
			}
            return true;
		}
        private void LoadProfile(bool blnForceDlg)
        {
            ...
            if ( !firstTimeLoad && blnForceDlg ) 
            {
                 if (LoadProfileInternal(false, ref messages)) return;
            }
            while(maxIterations >= ++count)
            {
                if (LoadProfileInternal(blnForceDlg, ref messages)) return;
            }

			if ( DialogResult.Retry == MessageBox.Show( "ProfileManager Form was unable to load profile. Retry?", "Startup Error", MessageBoxButtons.RetryCancel ) )
			{
				LoadProfile( blnForceDlg );
			}
		}
        ...
        private int GetNodeId()
        {
            ...
            NpgsqlCommand cmdGetNode = new(strSqlGet, connDb, t);
        ...
        private void LoadServerSettings()
        {
            ...
            NpgsqlDataAdapter daTemp = new(strSql, connDb);
        ...
        private void LoadDirData()
        {
            ...
            DirectoryInfo dirBase = new(strLocalFileRoot);
        ...
        private void LoadPropertyMaps()
        {
            ...
            string strSql = @"
                ...
                where active=true;
            ";
            NpgsqlCommand command = new(strSql, connDb);
        ...
        private void ResetView(string strTreePath = "")
        {
            ...
            if (lstEntrySel == null) lstEntrySel = [];
            int intLastIndex = 0;
        ...
        protected void PopulateTree(TreeNode tnParent)
        {
            ...
            foreach (DataRow dr in drShows)
            {
                ...
                // create the node
                TreeNode tnChild = new(dr.Field<string>("dir_name"))
                {
                    Tag = dr.Field<object>("dir_id")
                };
        ...
        protected void PopulateList(TreeNode nodeCurrent)
        {
            ...
            if (dsList.Tables[0] != null)
            {
                foreach (DataRow row in dsList.Tables[0].Select("", "entry_name asc"))
                {
                    ...
                    string[] lvData =
                    [
                        row.Field<string>("entry_name"), // Name
                        row.Field<string>("str_latest_size"), // Size
                        row.Field<string>("file_ext"), // Type
                        row.Field<string>("client_status_code"), // Stat
                        row.Field<string>("str_local_stamp"), // Local Modified
                        row.Field<string>("str_latest_stamp"), // Server Modified
                        strCheckout, // CheckOut
                        row.Field<string>("cat_name"), // Category
                        row.Field<string>("absolute_path") + "\\" + row.Field<string>("entry_name"), // FullName
                    ];
                    ...
                    ListViewItem lvItem = new(lvData)
                    {
                        ImageKey = strFileExt + strOverlay
                    };
        ...
        protected void PopulateHistoryPage(string FileName)
        {
            ...
            foreach (DataRow row in dsTemp.Tables[0].Rows)
            {
                ...
                string[] lvData =
                [
                    row.Field<int>("version_id").ToString(),
                    row.Field<string>("action_user"),
                    row.Field<string>("action_date"),
                    row.Field<string>("version_size"),
                    row.Field<string>("release_stamp"),
                ];
        ...
        protected void PopulateParentsPage(string FileName)
        {
            ...
            foreach (DataRow row in dsTemp.Tables[0].Rows)
            {
                ...
                string[] lvData =
                [
                    row.Field<int>("rel_child_id").ToString(),
                    row.Field<string>("entry_name") + " (v" + row.Field<int>("rel_parent_id").ToString() + ")",
                ];
        ...
        protected void PopulateChildrenPage(string FileName)
        {
            ...
            if (dr.Field<bool>("is_remote") == false || dr.Field<string>("client_status_code") == "cm")
            {
                ...
                {
                    SWDocMgr docMgr = new(strSWDocMgrKey);
        ...
        protected void PopulatePropertiesPage(string FileName)
        {
            ...
            foreach (DataRow row in dsTemp.Tables[0].Rows)
            {
                ...
                [
                    row.Field<int>("version_id").ToString(),
                    row.Field<string>("config_name"),
                    row.Field<string>("prop_name"),
                    strValue,
                    row.Field<string>("prop_type"),
                ];
        ...
        void worker_GetLatest(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < intUpdateCount; i++)
            {
                ...
                FileInfo fiCurrFile = new(strFullName);
        ...
        void worker_Commit(object sender, DoWorkEventArgs e)
        {
            ...
            if (blnFailed == false)
            {
                ...
                if (intRowCount != 0)
                {
                    ...
                    NpgsqlCommand cmdUpdateEntry = new(strSql, connDb, t);
        ...
        void worker_CheckOut(object sender, DoWorkEventArgs e)
        {
            ...
            NpgsqlCommand cmdCheckOut = new(strSql, connDb, t);
        ...
        private bool LoadCommitsDataRecurse(object sender, DoWorkEventArgs e, ref DataSet dsCommits, string strRelativePath)
        {
            ...
            foreach (string strFile in strFiles)
            {
                ...
                FileInfo fiCurrFile = new(strFile);
                ...
                foreach (DataRow row in drNeedsDepends)
                {
                    ...
                    if (lstDepends != null)
                    {
                        ...
                        foreach (string[] strDepends in lstDepends)
                        {
                            ...
                            FileInfo fiCurrFile = new(strLongName);
        ...
        private bool AddRemoteDirs(object sender, DoWorkEventArgs e, NpgsqlTransaction t, ref DataSet dsCommits)
        {
            ...
            NpgsqlCommand cmdGetId = new(strSql, connDb, t);
            cmdGetId.Prepare();
            ...
            ";
            NpgsqlCommand cmdInsert = new(strSql, connDb, t);
        ...
        private bool AddNewVersions(object sender, DoWorkEventArgs e, NpgsqlTransaction t, ref DataSet dsCommits)
        {
            ...
            NpgsqlCommand cmdGetEntryId = new(strSql, connDb, t);
            ...
            ";
            NpgsqlCommand cmdInsertEntry = new(strSql, connDb, t);
            ...
            NpgsqlCommand cmdGetVersionId = new(strSql, connDb, t);
            ...
            ";
            NpgsqlCommand cmdInsertVersion = new(strSql, connDb, t);
            ...
            for (int i = 0; i < intNewCount; i++)
            {
                ...
                FileInfo fiNewFile = new(strFullName);
            ...
            foreach (Tuple<string, string, string, object> tplProps in lstProps)
            {
                ...
                if (intPropId == 0)
                {
                    ...
                    else
                    {
                        ...
                        // get new property id
                        NpgsqlCommand cmdGetPropId = new("select nextval('seq_hp_entry_entry_id'::regclass);", connDb, t);
        ...
        private DataSet LoadFetchData(object sender, DoWorkEventArgs e, NpgsqlTransaction t, string strRelBasePath = null, List<int> lstSelected = null)
        {
            ...
            dsFetches = MatchLocalFiles(sender, e, dsFetches);
            blnFailed = false;
        ...
        private DataSet MatchLocalFiles(object sender, DoWorkEventArgs e, DataSet dsCommits)
        {
            // return true if something failed, false if successful
            ...
            int progressBarMax = dsCommits.Tables["files"].Rows.Count;
            int currentIndex = 0;

            //var rows = dsCommits.Tables["files"].AsEnumerable();





            //OdooView = new OdooViewer();
            //OdooView.Show();
            //OdooDefaults.Execute(OdooView.oClient, connDb, dsCommits.Tables["files"]);
            ...
            dlgStatus.SetProgressBar(0, progressBarMax);
            dlgStatus.DoubleBuff = true;
            ...
            foreach (DataRow drRemote in dsCommits.Tables["files"].Rows)
            {
                ...
                lock (lockObject)
                {
                    ...
                    dlgStatus.SetProgressBar(++currentIndex, progressBarMax);
        ...
        private DataSet LoadDeletesData(object sender, DoWorkEventArgs e, NpgsqlTransaction t, string strRelBasePath, List<string> lstSelectedNames)
        {
            ...
            if (lstSelectedNames == null && intBaseDirId != 0)
            {
                ...
                NpgsqlDataAdapter daTemp1 = new(strSql, connDb);
                ...
                NpgsqlDataAdapter daTemp2 = new(strSql, connDb);
                ...
                DirectoryInfo dirBase = new(strAbsBasePath);
        ...
        private bool DeleteLocalFiles(object sender, DoWorkEventArgs e, NpgsqlTransaction t, bool blnDeleteDirs, ref DataSet dsDeletes)
        {
            ...
            if (blnDeleteDirs)
            {
                ...
                if (Directory.Exists(strAbsPath))
                {
                    ...
                    DirectoryInfo dir = new(strAbsPath) { Attributes = FileAttributes.Normal };
            ...
            else
            {
                ...
                foreach (DataRow drFile in dsDeletes.Tables["files"].Rows)
                {
                    ...
                    string strFullName = strAbsolutePath + "\\" + strFileName;
                    FileInfo fiCurrent = new(strFullName);
        ...
        private bool DeleteRemoteEntries(object sender, DoWorkEventArgs e, NpgsqlTransaction t, bool blnDestroy, ref DataSet dsDeletes)
        {
            ...
            NpgsqlCommand cmdInactivateEntry = new(strSql, connDb, t);
        ...
        private bool DeleteRemoteDirs(object sender, DoWorkEventArgs e, NpgsqlTransaction t, ref DataSet dsDeletes)
        {
            ...
            NpgsqlCommand cmdInactivateDir = new(strSql, connDb, t);
        ...
        private bool UnDeleteRemoteEntries(object sender, DoWorkEventArgs e, NpgsqlTransaction t, ref DataSet dsDeletes)
        {
            ...
            NpgsqlCommand cmdActivateEntry = new(strSql, connDb, t);
        ...
        void CmsTreeGetLatestClick(object sender, EventArgs e)
        {
            ...
            List<object> arguments = [t, strRelBasePath, null];
        ...
        void CmsListGetLatestClick(object sender, EventArgs e)
        {
            ...
            // package arguments for the background worker
            List<object> arguments = [t, strRelBasePath, lstSelected];
        ...
        void CmsListCommitClick(object sender, EventArgs e)
        {
            ...
            // package arguments for the background worker
            List<object> arguments = [t, strRelBasePath, lstSelectedNames];
        ...
        void lvHistory_GetFile(object sender, EventArgs e)
        {
            ...
            NpgsqlCommand cmd = new(strSql, connDb);
            NpgsqlDataAdapter daTemp = new(strSql, connDb);
            ...
            {
                Filter = strTypeDesc + " (*" + strFileExt + ")|*" + strFileExt + "|All files (*)|*",
                Title = "Save File Version As...",
                InitialDirectory = strAbsPath,
                FileName = intOldVersionId + "." + strFileName,
                DefaultExt = strFileExt,
        ...
        void DoSearch()
        {
            ...
            SearchDialog srchdialog = new(connDb, strLocalFileRoot, intMyUserId, ShowFileInTree, StoreSearchParams, FileContainsText, PropDropDownText, PropContainsText, CheckedOutMeBox, DeletedLocalBox, LocalOnlyBox);
    ...
    public class ImgConvert : System.Windows.Forms.AxHost
    {
        ...
        public Image GetPicture(stdole.IPictureDisp inPic)
        {
            ...
            if (this.InvokeRequired)
            {
                ...
                GetPictureDel del = new(GetPicture);
