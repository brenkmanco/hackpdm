...
namespace HackPDM
{
    ...
    public partial class FileTypeManager : Form
    {
        ...
        public FileTypeManager(NpgsqlConnection dbConn, string FilePath, string TreePath) : this()
        {
            connDb = dbConn;
        ...
        private void LoadCategories()
        {
            ...
            Dictionary<Int32,string> items = new()
            {
                { 0, "" }
            };
            try
        ...
        private void LoadFilters()
        {
            ...
            daFilters = new NpgsqlDataAdapter(strSql, connDb)
            {
                ...
                InsertCommand = new NpgsqlCommand("insert into hp_entry_name_filter (name_proto, name_regex, description) values (:nameproto, :nameregex, :description)", connDb)
            };
        ...
        public Tuple<string,string,string,string> GetFileExt(string strFileName)
        {
            ...
            FileInfo fiCurrFile = new(strFileName);
        ...
        protected int GetRemoteType(string strFileName)
        {
            ...
            foreach (DataRow dr in dtRemTypes.Rows)
            {
                Regex rxExt = new(dr["type_regex"].ToString(), RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        protected bool FileBlocked(string strFileName)
        {
            ...
            foreach (DataRow dr in dsFilters.Tables[0].Rows)
            {
                Regex rxExt = new(dr["name_regex"].ToString(), RegexOptions.IgnoreCase);
                if (rxExt.IsMatch(strShortName))
                {
                    return true;
                }
        ...
        private void PopulateRemList()
        {
            ...
            foreach (DataRow row in dtRemTypes.Rows)
            {
                ...
                string[] lvData =
                [
                    row.Field<string>("file_ext"),
                    row.Field<string>("cat_name"),
                    row.Field<string>("type_regex"),
        ...
        private void PopulateLocList()
        {
            ...
            foreach (DataRow row in dtLocTypes.Rows)
            {
                ...
                // insert data
                string[] lvData = [row.Field<string>("file_ext"), row.Field<string>("status"), row.Field<string>("example")];
        ...
        private void btnTypesCommit_Click(object sender, EventArgs e)
        {
            ...
            NpgsqlCommand command = new("insert into hp_type (file_ext, default_cat, icon, type_regex, description) values (:fileext, :defaultcat, :icon, :typeregex, :description)", connDb);
