    ...
    public class WebDAVClient
    {
        ...
        public List<String> List(String remoteFilePath, int? depth)
        {
            ...
            using (HttpWebResponse response = (HttpWebResponse)httpWebRequest.GetResponse())
            {
                ...
                using (Stream stream = response.GetResponseStream())
                {
                    XmlDocument xml = new();
                    xml.Load(stream);
                    XmlNamespaceManager xmlNsManager = new(xml.NameTable);
                    ...
                    foreach (XmlNode node in xml.DocumentElement.ChildNodes)
                    {
                        ...
                        string[] file = filepath.Split([basePath], 2, StringSplitOptions.RemoveEmptyEntries);
                        if (file.Length > 0)
                        {
        ...
        public byte[] Download(String remoteFilePath)
        {
            ...
            using (HttpWebResponse response = (HttpWebResponse)httpWebRequest.GetResponse())
            {
                ...
                {
                    using (MemoryStream fs = new(contentLength))
                    {
                        int bytesRead = 0;
                        do
                        {
        ...
        public void Download(String remoteFilePath, String localFilePath)
        {
            ...
            using (HttpWebResponse response = (HttpWebResponse)httpWebRequest.GetResponse())
            {
                ...
                using (Stream s = response.GetResponseStream())
                {
                    using (FileStream fs = new(localFilePath, FileMode.Create, FileAccess.Write))
                    {
                        byte[] content = new byte[4096];
                        int bytesRead = 0;
                        do
        ...
        public async Task DownloadAsync(String remoteFilePath, String localFilePath)
        {
            // Should not have a trailing slash.
            Uri downloadUri = getServerUrl(remoteFilePath, false);
            string method = WebRequestMethods.Http.Get.ToString();
            ...
            using (HttpWebResponse response = (HttpWebResponse) await httpWebRequest.GetResponseAsync())
            {
                ...
                using (Stream s = response.GetResponseStream())
                {
                    ...
                    using (FileStream fs = new(localFilePath, FileMode.Create, FileAccess.Write, FileShare.Write, 4096, true))
                    {
                        byte[] content = new byte[4096];
                        int bytesRead = 0;
                        do
                        {
                            bytesRead = await s.ReadAsync(content, 0, content.Length);
                            await fs.WriteAsync(content, 0, bytesRead);
                        } while (bytesRead > 0);
                    }
                }
            }
        ...
        private void PushContentStream(byte[] content, string uploadFilePath)
        {
            using (Stream streamResponse = httpWebRequest.GetRequestStream()) {
                ...
                if (content != null) {
                    ...
                    using (FileStream fs = new(uploadFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                    {
